...
"""GPTQ-Int4 experiment: Qwen3.5-35B-A3B-GPTQ-Int4 on vLLM.

Runs Exp 1 (drift baseline) and Exp 2 (temperature ablation) for the GPTQ-Int4
...
Parallelism: 1 worker (single job at a time per user request).

Usage:
    nohup uv run python -u scripts/run_qwen35_gptq.py \
        > results/qwen35_gptq/run.log 2>&1 &
"""
...
MODEL_LABEL = "qwen3.5:35b-gptq"

# Output files
EXP1_DATA_PATH = Path("results/qwen35_gptq/exp1_drift_data.json")
EXP2_DATA_PATH = Path("results/qwen35_gptq/exp2_drift_data.json")
LOG_DIR = Path("results/qwen35_gptq")
...
def main() -> None:
    ...
    print(f"\n{'='*70}")
    print("All GPTQ-Int4 experiments complete!")
