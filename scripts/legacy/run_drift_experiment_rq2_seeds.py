...
"""Experiment B: Part 2 KV cache replication with seeds — 10 Qwen3 models × 5 seeds.

Run twice: once with --kv-mode q8, then user switches Ollama KV cache config,
then again with --kv-mode fp16. Data saved to separate files, merged by analysis script.

Uses ThreadPoolExecutor(max_workers=4) for parallelism.
Resumable: skips condition keys already present in the data file.

Usage:
    # First run (q8_0 KV cache — Ollama default):
    nohup uv run python -u scripts/run_drift_experiment_rq2_seeds.py --kv-mode q8 \
        > results/drift_experiment_rq2_seeds/run_q8.log 2>&1 &

    # User manually switches Ollama KV cache to fp16, then:
    nohup uv run python -u scripts/run_drift_experiment_rq2_seeds.py --kv-mode fp16 \
        > results/drift_experiment_rq2_seeds/run_fp16.log 2>&1 &
"""
...
# 10 key Qwen3 models covering biggest winners, losers, and stable from Part 2.
# All on <GPU-HOST-D> Ollama port 11434.
MODELS = {
    "qwen3:0.6b-fp16": {
...
OUT_DIR = Path("results/drift_experiment_rq2_seeds")
...
def build_condition_matrix() -> list[tuple[str, dict, int]]:
    """Build the condition matrix: 10 models × 5 seeds = 50 runs.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="RQ2 KV cache drift experiment with seeds")
    parser.add_argument(
        "--kv-mode",
        required=True,
        choices=["q8", "fp16"],
        help="KV cache precision mode (determines output filename)",
    )
    args = parser.parse_args()

    kv_mode = args.kv_mode
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    data_path = OUT_DIR / f"drift_data_{kv_mode}.json"

    print(f"KV cache mode: {kv_mode}")
    ...
    # Print summary
    print(f"\n{'='*70}")
    print(f"SUMMARY — Mean Final Similarity by Model ({kv_mode} KV cache, 5 seeds)")
    print(f"{'='*70}")
    print(f"{'Model':<45} {'Mean':>8} {'Std':>8} {'Min':>8} {'Max':>8}")
    print(f"{'-'*45} {'-'*8} {'-'*8} {'-'*8} {'-'*8}")
    ...
    for label, mean, std, mn, mx in rows:
        print(f"  {label:<43} {mean:>8.4f} {std:>8.4f} {mn:>8.4f} {mx:>8.4f}")
