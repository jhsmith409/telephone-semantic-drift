...
Usage:
    nohup uv run python -u scripts/run_nemo_30b.py \
        > results/qwen35_nemo/run.log 2>&1 &
"""
...
MODEL_LABEL = "nemo:30b-nvfp4"

# Output files — separate from other quant data
EXP1_DATA_PATH = Path("results/qwen35_nemo/exp1_drift_data.json")
EXP2_DATA_PATH = Path("results/qwen35_nemo/exp2_drift_data.json")
LOG_DIR = Path("results/qwen35_nemo")
...
def run_experiment(
    ...
    """Run an experiment with parallel workers."""
    print(f"\n{'='*70}")
    print(f"Nemotron {name}")
    print(f"{'='*70}")
...
def main() -> None:
    ...
    print(f"\n{'='*70}")
    print("All Nemotron experiments complete!")
