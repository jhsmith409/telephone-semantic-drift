...
"""NVFP4 system prompt experiment: Qwen3.5-35B-A3B-NVFP4 on sglang.

Runs the same 20 system prompts × 2 recipes × 5 seeds as Exp 3c (vLLM/AWQ)
and Exp 4 (Ollama), enabling a three-way GGUF/AWQ/NVFP4 comparison on
prompt sensitivity.
...
Usage:
    nohup uv run python -u scripts/run_qwen35_nvfp4_sysprompt.py \
        > results/qwen35_nvfp4/run_sysprompt.log 2>&1 &
"""
...
DATA_PATH = Path("results/qwen35_nvfp4/sysprompt_drift_data.json")
...
def main() -> None:
    Path("results/qwen35_nvfp4").mkdir(parents=True, exist_ok=True)
    ...
    if not remaining:
        print("All NVFP4 sysprompt conditions already complete.")
        return
    ...
    print(f"\n{'='*70}")
    print(f"NVFP4 sysprompt experiment complete! {completed} conditions saved.")
