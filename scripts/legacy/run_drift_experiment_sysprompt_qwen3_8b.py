...
"""Run system prompt ablation on qwen3:8b (q4_K_M, q8 KV cache) via Ollama.

Combined spaghetti (3-step) + lasagna (21-step) experiment.
20 prompts × 2 recipes × 5 seeds = 200 runs of 30 iterations each (6,000 API calls).
...
Usage:
    nohup uv run python -u scripts/run_drift_experiment_sysprompt_qwen3_8b.py &
"""
...
OUT_DIR = Path("results/drift_experiment_sysprompt_qwen3_8b")
...
def resolve_model(hostname: str = "gpu-host-d") -> dict:
    """Resolve hostname and return model info for qwen3:8b on Ollama."""
...
def build_condition_matrix(model_info: dict) -> list[tuple[str, dict, str, int, str, str]]:
    """Build the condition matrix: 2 recipes x 20 prompts x 5 seeds = 200 runs.

    Returns list of (condition_key, model_info, system_prompt, seed, recipe, initial_prompt).
    """
    conditions = []
    for recipe, initial_prompt in INITIAL_PROMPTS.items():
        for prompt in PROMPTS:
            slug = _slug(prompt["name"])
            base_key = f"{recipe}_{prompt['id']:02d}_{slug}"
            for seed in SEEDS:
                key = f"{base_key}_s{seed}"
                conditions.append((key, model_info, prompt["text"], seed, recipe, initial_prompt))

    return conditions


def plot_line(all_data: dict, recipe: str, out_path: Path, model_label: str) -> None:
    """Generate a line plot with mean +/- min/max bands across 5 seeds for one recipe."""
    import matplotlib
    matplotlib.use("Agg")
    ...
    ax.set_title(
        f"System Prompt Ablation — {recipe_label} — {model_label}\n"
        f"20 Prompts x {ITERATIONS} Iterations (paraphrase, t=0.7, 5 seeds)",
        fontsize=13,
    )
...
def plot_bar_comparison(all_data: dict, out_path: Path, model_label: str) -> None:
    """Generate grouped bar chart: spaghetti (solid) vs lasagna (hatched) for all 20 prompts."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    import numpy as np

    prompt_stats = []  # (name, category, mean_spag, std_spag, mean_las, std_las)
    ...
    for prompt in PROMPTS:
        ...
        mean_spag = np.mean(finals["spaghetti"]) if finals["spaghetti"] else 0
        std_spag = np.std(finals["spaghetti"]) if finals["spaghetti"] else 0
        mean_las = np.mean(finals["lasagna"]) if finals["lasagna"] else 0
        std_las = np.std(finals["lasagna"]) if finals["lasagna"] else 0

        prompt_stats.append((prompt["name"], prompt["category"], mean_spag, std_spag, mean_las, std_las))

    # Sort by lasagna mean descending
    prompt_stats.sort(key=lambda x: x[4], reverse=True)

    names = [s[0] for s in prompt_stats]
    categories = [s[1] for s in prompt_stats]
    means_spag = [s[2] for s in prompt_stats]
    stds_spag = [s[3] for s in prompt_stats]
    means_las = [s[4] for s in prompt_stats]
    stds_las = [s[5] for s in prompt_stats]
    ...
    ax.bar(x - width / 2, means_spag, width, yerr=stds_spag, capsize=3,
           color=colors, label="Spaghetti (3-step)", edgecolor="white", linewidth=0.5)
    ax.bar(x + width / 2, means_las, width, yerr=stds_las, capsize=3,
           color=colors, label="Lasagna (21-step)", alpha=0.6, hatch="//",
           edgecolor="white", linewidth=0.5)

    # Baseline reference line (lasagna baseline)
    baseline_las_mean = None
    for name, _, _, _, m_las, _ in prompt_stats:
        if name == "Baseline - None":
            baseline_las_mean = m_las
            break
    if baseline_las_mean is not None:
        ax.axhline(y=baseline_las_mean, color="#888888", linestyle="--", alpha=0.7,
                    label=f"Baseline lasagna ({baseline_las_mean:.2f})")

    ax.set_xlabel("System Prompt", fontsize=11)
    ax.set_ylabel("Mean Final Cosine Similarity (+/- 1 std)", fontsize=11)
    ax.set_title(
        f"System Prompt Ablation — Spaghetti vs Lasagna — {model_label}\n"
...
def main() -> None:
    ...
    model_info = resolve_model("gpu-host-d")
    model_label = "qwen3:8b-q4_K_M, q8 KV cache"
    ...
    remaining = [(k, mi, sp, s, r, ip) for k, mi, sp, s, r, ip in conditions if k not in all_data]
    print(f"Total conditions: {total}")
    print(f"Already completed: {total - len(remaining)}")
    print(f"Remaining: {len(remaining)}")
    print(f"Model: {model_info['model']} on {model_info['host']}:{model_info['port']}")
    ...
    if not remaining:
        ...
        def _run_and_save(args: tuple) -> str:
            key, mi, sp, seed, recipe, initial_prompt = args
            cond_key, results = run_single_chain(key, mi, sp, seed, initial_prompt)
    ...
    # Generate plots
    plot_line(all_data, "spaghetti", OUT_DIR / "drift_plot_spaghetti.png", model_label)
    plot_line(all_data, "lasagna", OUT_DIR / "drift_plot_lasagna.png", model_label)
    plot_bar_comparison(all_data, OUT_DIR / "drift_bar_comparison.png", model_label)

    # Print summary table
    print(f"\n{'='*70}")
    print(f"SUMMARY — Mean Final Similarity by Prompt ({model_label})")
    print(f"{'='*70}")
    print(f"{'Prompt':<35} {'Category':<14} {'Spaghetti':>10} {'Lasagna':>10}")
    print(f"{'-'*35} {'-'*14} {'-'*10} {'-'*10}")
    ...
    for prompt in PROMPTS:
        ...
        mean_spag = sum(finals["spaghetti"]) / len(finals["spaghetti"]) if finals["spaghetti"] else 0
        mean_las = sum(finals["lasagna"]) / len(finals["lasagna"]) if finals["lasagna"] else 0
        rows.append((prompt["name"], prompt["category"], mean_spag, mean_las))

    # Sort by lasagna mean descending
    rows.sort(key=lambda x: x[3], reverse=True)
    for name, category, mean_spag, mean_las in rows:
        print(f"  {name:<33} {category:<14} {mean_spag:>10.4f} {mean_las:>10.4f}")
