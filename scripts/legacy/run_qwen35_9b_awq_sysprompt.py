...
"""9B AWQ system prompt experiment: Qwen3.5-9B-AWQ-4bit on vLLM.

Runs the same 20 system prompts × 2 recipes × 5 seeds as the other sysprompt
experiments, enabling AWQ vs GGUF comparison at 9B scale and dense vs MoE
prompt sensitivity analysis.

20 prompts × 2 recipes × 5 seeds × 30 iters = 6,000 API calls

vLLM endpoint: <GPU-HOST-A>:8005 (OpenAI-compatible, non-thinking).

Usage:
    nohup uv run python -u scripts/run_qwen35_9b_awq_sysprompt.py \
        > results/qwen35_9b_awq/run_sysprompt.log 2>&1 &
"""
...
# Same 20 system prompts as all other sysprompt experiments
SYSTEM_PROMPTS = {
...
DATA_PATH = Path("results/qwen35_9b_awq/sysprompt_drift_data.json")
...
def main() -> None:
    Path("results/qwen35_9b_awq").mkdir(parents=True, exist_ok=True)
    ...
    # Build conditions: 20 prompts × 2 recipes × 5 seeds = 200
    conditions = []
    ...
    if not remaining:
        print("All 9B AWQ sysprompt conditions already complete.")
        return
    ...
    print(f"\n{'='*70}")
    print(f"9B AWQ sysprompt experiment complete! {completed} conditions saved.")
