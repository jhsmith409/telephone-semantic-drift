...
"""Experiment 1: Qwen3.5 size × quantization drift.

6 sizes (0.8B, 2B, 4B, 9B, 27B, 35B) × 3 quants (q4_K_M, q8_0, bf16)
+ 122B q4_K_M only = 18 models × 2 recipes × 5 seeds × 30 iterations.

Both spaghetti (3-step) and lasagna (21-step) prompts, T=0.7, paraphrase style.

Usage:
    nohup uv run python -u scripts/run_qwen35_size_quant.py \
        > results/qwen35_size_quant/run.log 2>&1 &
"""
...
# Qwen3.5 models: 6 sizes × 3 quants + 122B q4_K_M = 19 models
# Note: 0.8B has no q4_K_M on Ollama — default tag is Q8_0, so we use q8_0 + bf16 only (+ q8_0 as stand-in)
MODELS = {
    # 0.8B (no q4_K_M available — only q8_0 and bf16)
    ...
    # 122B q4_K_M only (too large for q8/bf16)
...
def build_condition_matrix() -> list[tuple[str, dict, int, str]]:
    """Build condition matrix: 18 models × 2 recipes × 5 seeds.

    Spaghetti keys: "{model}_s{seed}" (backward-compatible with existing runs).
    Lasagna keys: "lasagna_{model}_s{seed}".
    Returns list of (condition_key, model_info, seed, initial_prompt).
    """
    conditions = []
    for label, model_info in MODELS.items():
        for seed in SEEDS:
            # Spaghetti (original key format for backward compat)
            key = f"{label}_s{seed}"
...
def main() -> None:
    ...
    for recipe in ["spaghetti", "lasagna"]:
        ...
        print(f"\n{'='*70}")
        print(f"SUMMARY — {recipe.upper()} — Mean Final Similarity by Model (5 seeds)")
        print(f"{'='*70}")
