...
"""GPTQ-Int4 system prompt experiment: Qwen3.5-35B-A3B-GPTQ-Int4 on vLLM.

Runs the same 20 system prompts × 2 recipes × 5 seeds as Exp 3c (vLLM/AWQ),
Exp 4 (Ollama), and NVFP4, enabling a four-way GGUF/AWQ/NVFP4/GPTQ comparison
on prompt sensitivity.

20 prompts × 2 recipes × 5 seeds × 30 iters = 6,000 API calls

vLLM endpoint: <GPU-HOST-D>:8005 (OpenAI-compatible).

Usage:
    nohup uv run python -u scripts/run_qwen35_gptq_sysprompt.py \
        > results/qwen35_gptq/run_sysprompt.log 2>&1 &
"""
...
DATA_PATH = Path("results/qwen35_gptq/sysprompt_drift_data.json")
...
def main() -> None:
    Path("results/qwen35_gptq").mkdir(parents=True, exist_ok=True)
    ...
    if not remaining:
        print("All GPTQ-Int4 sysprompt conditions already complete.")
        return
    ...
    print(f"\n{'='*70}")
    print(f"GPTQ-Int4 sysprompt experiment complete! {completed} conditions saved.")
