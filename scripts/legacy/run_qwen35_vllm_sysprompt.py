...
"""Qwen3.5 35B AWQ system prompt ablation on vLLM.

20 system prompts × 2 recipes × 5 seeds = 200 conditions × 30 iterations.

Usage:
    nohup uv run python -u scripts/run_qwen35_vllm_sysprompt.py \
        > results/qwen35_vllm_sysprompt/run.log 2>&1 &
"""
...
OUT_DIR = Path("results/qwen35_vllm_sysprompt")
...
def build_condition_matrix() -> list[tuple[str, str, int, str]]:
    """Build condition matrix: 20 prompts × 2 recipes × 5 seeds = 200 runs.

    Key format: "{recipe}_{id:02d}_{slug}_s{seed}"
    Returns list of (condition_key, system_prompt, seed, initial_prompt).
    """
    conditions = []
    for recipe, initial_prompt in INITIAL_PROMPTS.items():
        for sp in SYSTEM_PROMPTS:
            slug = _slug(sp["name"])
            for seed in SEEDS:
                key = f"{recipe}_{sp['id']:02d}_{slug}_s{seed}"
                conditions.append((key, sp["text"], seed, initial_prompt))
    return conditions
...
def main() -> None:
    ...
    remaining = [(k, sp, s, ip) for k, sp, s, ip in conditions if k not in all_data]
    ...
    if not remaining:
        ...
        def _run_and_save(args: tuple) -> str:
            key, sp, seed, ip = args
            cond_key, results = run_single_chain(key, sp, seed, ip)
            with data_lock:
    ...
    for recipe in ["spaghetti", "lasagna"]:
        print(f"\n{'='*70}")
        print(f"SUMMARY — {MODEL_LABEL} — {recipe.upper()}")
        print(f"{'='*70}")
