...
"""AWQ-4bit experiment: Qwen3.5-9B-AWQ-4bit on vLLM.

Runs Exp 1 (drift baseline) and Exp 2 (temperature ablation) for the 9B AWQ
quantization variant, enabling AWQ vs GGUF comparison at a second model size
(complementing the 35B MoE comparison).
...
vLLM endpoint: <GPU-HOST-A>:8005 (OpenAI-compatible).
Parallelism: 1 worker (single job at a time).

Usage:
    nohup uv run python -u scripts/run_qwen35_9b_awq.py \
        > results/qwen35_9b_awq/run.log 2>&1 &
"""
...
MODEL_LABEL = "qwen3.5:9b-awq"

# Output files
EXP1_DATA_PATH = Path("results/qwen35_9b_awq/exp1_drift_data.json")
EXP2_DATA_PATH = Path("results/qwen35_9b_awq/exp2_drift_data.json")
LOG_DIR = Path("results/qwen35_9b_awq")
...
def run_experiment(
    ...
    """Run an experiment sequentially (1 worker)."""
    print(f"\n{'='*70}")
    print(f"9B AWQ {name}")
    print(f"{'='*70}")
...
def main() -> None:
    ...
    print(f"\n{'='*70}")
    print("All 9B AWQ experiments complete!")
