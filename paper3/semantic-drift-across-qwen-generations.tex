...
llama.cpp, we find:
(1)~cosine similarity to the original, the drift measure used by this line
of work, does not track the information a document carries: across sixteen
domains with ten checkable facts each it is only weakly related to the
survival of those facts ($\rho=0.32$), and on construction statements of
work it is negatively related ($\rho=-0.42$), because outputs inflate two-
to seven-fold and reframe as memos while every number survives; dates are
the most fragile fact type (35\% survival, on six date anchors) and
identifiers the most robust (81\%);
(2)~seeds do not pin chains on any of these stacks---identical requests
...
(3)~thinking mode is a large effect whose sign depends on the model, costing
...
runaway-thinking failure of the previous generation at a low rate, which a
presence penalty of 1.5 in the sampler removed in the one condition tested
without changing the output otherwise;
(4)~multi-token-prediction speculative decoding shows no mean shift we can
detect at 15 samples, and temperature~0 narrows the spread without moving
the mean;
(5)~system-prompt rankings replicate across hosts ($\rho=0.95$--$0.98$) and
transfer across models within this generation ($\rho=0.70$--$0.90$), where
the earlier work found no transfer across families or sizes, and one
domain-neutral fidelity prompt raises every one of sixteen domains to
0.88--0.98; and
(6)~most chains lock byte-for-byte within ten iterations, so ``drift'' on
these models is largely where a copying attractor caught the chain.
We conclude that similarity-based drift scores measure genre and length
change far more than information loss, and we release the full chain texts
so that other metrics can be applied.
\end{abstract}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item A cross-generation comparison at matched size from Qwen3 through
        ...
        worsened after Qwen3.5 as models grew more verbose, while drift on
        the long stimulus improved, with the caveat that the size matching is
        approximate and the older anchors rest on few chains
  ...
  \item No detectable mean effect of speculative decoding or of greedy
        decoding on drift at 15 samples, greedy decoding acting only on the
        spread, and a re-test of the format-and-stack contrast on two new
        generations that reproduces its magnitude on one model and not its
        direction (Sections~\ref{sec:format} and \ref{sec:temperature}).
...
chen2023speculative} is distribution-preserving in principle, a property of
the algorithm rather than of any serving stack's numerics; multi-token
prediction heads, introduced as a training objective
\citep{gloeckle2024multitoken} and used by DeepSeek-V3 as a self-speculative
draft~\citep{deepseek2024v3}, are what the llama.cpp MTP arm exercises.
Section~\ref{sec:format} tests empirically whether drift is unchanged on
that stack. Instruction-following
...
\emph{spaghetti}, long \emph{lasagna} of nominally 21 steps, reproduced in
...
baseline. On the long lasagna recipe every configuration finishes between
...
\input{tables/table2_cross_generation}
...
\input{tables/table3_determinism}
...
GGUF, is not reproduced; a gap of comparable magnitude appears on one of the
two models, in the opposite direction. This is a re-test under a changed
stack, not a replication.
...
\input{tables/table5_format_stack}

\paragraph{Speculative decoding: no detectable effect.}
...
bytes, same server, one flag. Figure~\ref{fig:mtp} shows no detectable difference
(spaghetti 0.594, CI [0.48, 0.70], vs.\ 0.532, CI [0.41, 0.65], $p=0.44$;
lasagna 0.878 vs.\ 0.897, $p=0.12$; 15 samples each). At this sample size
the comparison cannot distinguish a shift of about 0.06 from zero on the
short stimulus, so the result is an absence of evidence for an effect above
the noise floor, not a demonstrated equivalence. That is consistent with
draft-and-verify sampling being distribution-preserving in
theory~\citep{leviathan2023speculative,chen2023speculative}, though the
theory concerns the algorithm and this test concerns one stack's numerics.
An earlier five-sample reading of the same comparison had suggested a large
penalty; it did not survive replication, which is the point of
...
$T=0.7$ and recovering to 0.747 at $T=1.0$; unlike the low-temperature
collapses Paper~A reports for small Qwen3.5 models, which are runaway-thinking
rates rather than drift, this is a genuine non-monotonicity in a
thinking-off model with no runaway chains. Flash-Next declines with
...
0.95 (lasagna), and the agreement is not carried by a few extreme prompts:
dropping the best and worst prompt leaves $\rho$ at 0.98 and 0.93, the
smallest value over every leave-two-out pair is 0.97 and 0.93, and removing
the whole visually oriented category gives 0.97 and 0.92. Across models
within the same recipe the rankings agree at 0.83--0.90 on spaghetti and
0.70--0.88 on lasagna; across recipes within a model at only 0.25--0.58. A prompt ranking is thus a real, measurable property of a
...
order kept 13\%. The date figure rests on the six date anchors in the set,
one in each of six domains, so it is a consistent signal across those six
documents rather than a large-sample estimate; the other three types rest on
28 to 96 anchors. Meta-commentary framing is present in 70--97\% of final
...
$p=0.29$) but reduces anchor survival (0.60 against 0.68), and at five
samples per domain its largest losses fall on literary prose ($-0.20$) and
the recipe ($-0.15$), a directional observation rather than a per-domain
result. The Ultra-Constrained Fidelity prompt,
...
\input{tables/table8_breadth_domain}

\input{tables/table9_breadth_facttype}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Do not expect seeds or speculative decoding to change drift; at 15
        samples neither moved the mean. Greedy decoding narrows the spread of
        outcomes without improving the mean, which matters only if the
        worst chain is what you care about.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Anchor facts are hand-chosen.} Ten per domain, chosen by the
        ...
        (``8 minutes'' rewritten as ``eight minutes'' counts as lost), the
        domain prompts are synthetic documents rather than sampled ones, the
        type labels are approximate (the literary anchors tagged as names are
        phrases), and the date type has only six anchors.
...
sauce.'' \textbf{Lasagna}: the beef lasagna recipe of Paper~A, nominally 21 steps,
...
with the chain data attached to the repository's data releases as
checksummed archives. The same data, code and paper sources are archived at
Zenodo: the dataset record has DOI \href{https://doi.org/10.5281/zenodo.22646927}{10.5281/zenodo.22646927},
this paper \href{https://doi.org/10.5281/zenodo.22646931}{10.5281/zenodo.22646931}, and Paper~A
\href{https://doi.org/10.5281/zenodo.22646929}{10.5281/zenodo.22646929}.
