...
repetition. The rates below should be read as properties of reasoning-enabled
Qwen3.5 under each stack's default sampler. The companion paper tests the
sampler directly on the following generation and finds that adding the
vendor-recommended presence penalty of 1.5 eliminates the failure (0 of 15
chains against 8 of 15) while removing top-$k$ and top-$p$ does not, so the
stack differences here are most plausibly differences in default sampling
of the reasoning block. Within the Ollama sweep the rate
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Serving stacks and samplers.} Results cover Ollama, vLLM
        ...
        than temperature (top-$p$, top-$k$, penalties) were left at each
        server's defaults and differ across stacks; they plausibly modulate
        the runaway rate and were not controlled.
